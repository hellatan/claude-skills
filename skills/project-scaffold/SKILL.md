...
Initialize git, **verify the release-please manifest invariant** (`package.json` / `pyproject.toml` / `.release-please-manifest.json` all read `0.0.1` — abort if any differ; `0.0.1` not `0.0.0`, to dodge release-please's `1.0.0` first-release bootstrap — see `references/step-15-git-init.md`), activate pre-commit hooks (deferred from Step 13 since `pre-commit install` needs `.git/` to exist), apply auto-fixers to the working tree, then commit so the initial commit lands clean. Without the pre-commit pass, every scaffold ships with a no-op fixup PR for trailing newlines and prettier nits — generators like `create-next-app` produce files that don't satisfy the hooks, and the bootstrap-exception contract (Step 17) prevents pushing those fixes directly to `develop`. The version check catches missed resets in 5 seconds rather than at the first release attempt. Create `main` + `develop` (+ `stage` if opted in) and check out `develop`.
