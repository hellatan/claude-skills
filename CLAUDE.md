...
- `scripts/test-release-steps.sh` (`npm run test:release-steps`) — **executes** the `Evaluate release outcome` step embedded in `skills/gh-actions-init/references/release-verification.md`, and lints the fenced yaml blocks around it. That one file only; see `tests/release-steps/README.md`
- `tests/release-steps/` — extractor, case driver + mutants, and workflow/shell lint for that step
- `package.json` — command surface only (`npm test` = validate + the step suite, and CI runs exactly that). No npm dependencies, no build; release-please uses the `simple` release type and never touches it.
